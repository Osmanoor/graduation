...
% TODO: Add heatmap figure
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2em}
    \textbf{[Figure placeholder: BM25 nDCG@10 Repetition Heatmap]}\\[0.5em]
    \small 9-row (models) $\times$ 8-column (repetition configs) heatmap with colour scale from 0.34 to 0.59, showing optimal region per model.
    \vspace{2em}}}
    \caption{Heatmap of BM25 nDCG@10 across nine models and eight repetition configurations. Darker shading indicates higher performance. The optimal region shifts rightward (higher repetition) for smaller models.}
    \label{fig:repetition_heatmap}
\end{figure}
...
% TODO: Add hybrid comparison figure
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2em}
    \textbf{[Figure placeholder: Hybrid Fusion Comparison]}\\[0.5em]
    \small Line plot of CC nDCG@10 vs. $\alpha$ with RRF $k=20$ and $k=60$ shown as horizontal dashed lines. BM25 and mDPR baselines shown as horizontal dotted lines.
    \vspace{2em}}}
    \caption{Hybrid fusion nDCG@10 as a function of CC $\alpha$. The horizontal dashed lines indicate RRF $k=20$ (0.6267) and RRF $k=60$ (0.6230). The dotted lines show the BM25 (0.4621) and mDPR (0.4993) baselines.}
...
BM25+CSQE achieved 0.6157 nDCG@10, representing a +33.2\% improvement over BM25 alone and, notably, nearly matching the no-QE hybrid baseline (0.6267) using only a single retriever. Dense+CSQE improved over mDPR alone by +18.5\% but fell below BM25+CSQE. The long CSQE expansion ($\sim$1,500 characters) degraded Dense retrieval quality relative to vocabulary-based matching, consistent with mDPR performing best on short queries comparable to those in its training data. This observation motivated the retriever-specific application experiments in Section~\ref{sec:res_csqe_hybrid}.
...
BM25-expanded was the winning configuration despite giving the Dense retriever the weaker input (the original short query rather than the CSQE expansion). This result is explained by a retriever--query representation mismatch: BM25 uses exact term matching and directly benefits from CSQE's vocabulary-rich expansion; the mDPR encoder produced less discriminative embeddings when given a $\sim$1,500-character expansion than with the original short query. Applying query expansion asymmetrically---only to BM25---maximised the gains from each retriever's inherent strength.

Both-expanded (both retrievers expanded) scored 0.6936, lower than BM25-expanded (0.7137), even though Dense+CSQE individually outperformed Dense alone (Section~\ref{sec:res_csqe_main}). This confirmed that the relative quality of the two ranked lists matters for RRF: a less discriminative Dense run reduced the fusion ceiling even when it improved individually. This finding illustrates a \textbf{retriever-specific query representation principle}: each retriever should receive the query format best suited to its scoring mechanism.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lcc@{}}
        ...
        \midrule
        Both-expanded RRF vs BM25-expanded RRF & $-$0.0201 & Dense+CSQE hurts in fusion \\
...
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2em}
    \textbf{[Figure placeholder: Per-Query CSQE vs. Blind Scatter Plot]}\\[0.5em]
    \small Each point is one query; $x$-axis = Blind nDCG@10, $y$-axis = CSQE+Hybrid nDCG@10. Points above diagonal = CSQE wins; below = regressions. Colour by first-pass quality (relevant/not relevant).
    \vspace{2em}}}
    \caption{Per-query nDCG@10 scatter plot comparing CSQE+Hybrid (Both-expanded RRF) against the Blind BM25 baseline. Points above the diagonal indicate queries where CSQE improved retrieval; points below indicate regressions.}
...
When first-pass retrieval succeeded (36.6\% of queries), CSQE+Hybrid achieved 0.8877 nDCG@10---near-ceiling retrieval. The first-pass quality gap (+0.3063 between the first-pass success and failure groups) was larger than the overall system advantage over blind QE (+0.1890), confirming that first-pass retrieval quality is the primary driver of CSQE effectiveness. Short queries ($<$5 words, $n=865$) gained most from CSQE (+0.1990), as they are the most underspecified and benefit from both vocabulary expansion and corpus grounding. Long queries ($\geq$10 words, $n=131$) still gained (+0.1053) but less, because their greater information content already mitigated the information-poverty problem that motivates query expansion.
...
% TODO: Add regression pie chart figure
\begin{figure}[htbp]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2em}
