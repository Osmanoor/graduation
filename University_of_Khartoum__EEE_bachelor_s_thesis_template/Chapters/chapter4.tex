...
A striking finding was that 39\% of queries (1,130 out of 2,896) were classified as failed, with 192 queries achieving zero recall in the top-10 (no relevant passages retrieved). Only 5.1\% of queries were classified as highly successful. This high failure rate established a clear opportunity for query enhancement to make a measurable impact. Figure~\ref{fig:failure_cliff} visualises this failure cliff on the cumulative distribution of per-query NDCG@10.
...
\begin{table}[htbp]
    \centering
    \caption{Per-query comparison of the CSQE+Hybrid system (Both-expanded RRF, 0.6936 nDCG@10) against the Aya Blind BM25 $n=1$ baseline (0.5046 nDCG@10) on 2,896 MIRACL Arabic dev queries.}
...
The CSQE+Hybrid system improved on 56.8\% of queries and regressed on 16.6\%, with a mean per-query delta of +0.1890 nDCG@10. The 26.6\% of ties were predominantly queries where both systems achieved either 0.0 (irretrievable) or 1.0 (perfectly retrieved).

A total of 258 queries (8.9\%) were classified as failures (CSQE nDCG@10 $< 0.1$). Inspection revealed that 257 of these 258 queries were irretrievable regardless of the QE method employed---the relevant passage was absent from the Wikipedia corpus dump used for indexing. These represented a dataset-level ceiling, not a CSQE failure. The remaining case involved the LLM generating a meta-description rather than vocabulary-rich content, a known failure mode of generative query expansion.
...
\subsection{First-Pass Quality as Dominant Predictor}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lcccc@{}}
        \toprule
        \textbf{Query Length} & \textbf{$n$} & \textbf{Blind QE nDCG@10} & \textbf{CSQE+Hybrid nDCG@10} & $\Delta$ \\
        \midrule
        Short ($<$5 words) & 865 & 0.4793 & 0.6783 & +0.1990 \\
        Medium (5--9 words) & 1,900 & --- & --- & --- \\
        Long ($\geq$10 words) & 131 & 0.6003 & 0.7056 & +0.1053 \\
        \bottomrule
    \end{tabular}
\end{table}

When first-pass retrieval succeeded (36.6\% of queries), CSQE+Hybrid achieved 0.8877 nDCG@10---near-ceiling retrieval. The first-pass quality gap (+0.3063 between the first-pass success and failure groups) was larger than the overall system advantage over blind QE (+0.1890), confirming that first-pass retrieval quality is the primary driver of CSQE effectiveness. Figure~\ref{fig:first_pass_split} visualises this split. Short queries ($<$5 words, $n=865$) gained most from CSQE (+0.1990), as they are the most underspecified and benefit from both vocabulary expansion and corpus grounding. Long queries ($\geq$10 words, $n=131$) still gained (+0.1053) but less, because their greater information content already mitigated the information-poverty problem that motivates query expansion. Figure~\ref{fig:length_gain} shows the per-bucket comparison.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}p{3.5cm}p{3.5cm}p{3.5cm}c@{}}
        \toprule
        \textbf{Query} & \textbf{Blind QE generates} & \textbf{CSQE grounds to} & \textbf{CSQE} \\
        \midrule
...
This pattern validated the corpus grounding hypothesis: for niche or ambiguous Arabic terms, blind QE confidently generated a common modern interpretation, while corpus grounding locked onto the actual Wikipedia entity. The impact on query expansion was direct: the corpus anchor provided the correct Arabic vocabulary (e.g., \<مملوكي>، \<قاهرة> rather than \<جراحة العمود الفقري>) that both BM25 and Dense scoring relied upon.
...
Type~B regressions (36\%, 131 queries) occurred when BM25 was sensitive to Arabic homonyms and short or ambiguous queries. A concrete example was the query ``\<ماهو التطرف؟>'' (``What is extremism?''): BM25 retrieved ``\<لهجة جنوبية>'' (a southern dialect article) because ``\<ماهو>'' matched a dialectal phrase rather than the intended interrogative ``\<ما هو>'' (``what is''). The LLM then generated corpus-grounded content about dialects, poisoning the expansion for the actual query intent.

\paragraph{Implications.}
These two failure modes suggested concrete future improvements: (1)~a first-pass quality gate that falls back to blind QE when the top-1 BM25 document has low lexical overlap with the query, which would address Type~B regressions; and (2)~asymmetric expansion weighting that applies lower weight for the expansion in the concatenated query when BM25 is already strong, which would address Type~A regressions.
