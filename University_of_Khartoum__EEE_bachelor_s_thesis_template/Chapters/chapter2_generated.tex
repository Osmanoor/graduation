...
Retrieval-Augmented Generation (RAG) addresses the hallucination problem by augmenting LLMs with external knowledge retrieval \cite{lewis2020retrieval}. The standard RAG architecture consists of two primary components:
...
\begin{itemize}
    ...
    \item \textbf{Dense Retrieval:} Neural methods that encode queries and documents into dense vector representations, enabling semantic similarity matching \cite{karpukhin2020dense}
...
Recent research has highlighted the sensitivity of RAG systems to query formulation. Studies on RAG robustness demonstrate that minor variations in query phrasing can significantly impact retrieval quality \cite{robustness_rag}. Query noise can originate from multiple sources:
...
The QE-RAG framework specifically addresses query noise, demonstrating that query enhancement techniques can significantly improve retrieval robustness \cite{qe_rag}. Crucially, the concept of "noise" in English queries (primarily typos and paraphrasing) is analogous to dialectal variations in Arabic—both represent deviations from the standard form present in the knowledge base.
...
\textbf{Hierarchical Structures:} RAPTOR \cite{raptor} introduces recursive abstractive processing, organizing documents into tree structures where higher levels contain summaries of lower levels. This enables retrieval at multiple levels of abstraction.

\textbf{Graph-Based Indexing:} LightRAG \cite{lightrag} and similar approaches represent knowledge as graphs, capturing relationships between entities and concepts. Graph structures enable multi-hop reasoning and relationship-aware retrieval.

\textbf{Proposition-Based Chunking:} Dense X Retrieval \cite{densex} decomposes documents into atomic propositions, creating fine-grained retrieval units that reduce context dilution.
...
\textbf{Self-Reflective RAG:} Approaches like Self-RAG \cite{selfrag} that incorporate critique tokens, enabling the model to assess retrieval quality and decide when additional retrieval is needed.
...
\textbf{HyDE (Hypothetical Document Embeddings):} Proposed by Gao et al. \cite{hyde}, HyDE instructs an LLM to generate a hypothetical document that would answer the query, then uses this generated document for retrieval instead of the original query. The intuition is that document-to-document similarity is more reliable than query-to-document similarity. HyDE operates in a zero-shot manner, requiring no training data, and has shown effectiveness for both sparse (BM25) and dense retrieval methods.

\textbf{Query2Doc:} Extends the HyDE concept by generating multiple pseudo-documents and combining them with the original query, creating a richer representation for retrieval \cite{query2doc}.
...
\textbf{Rewrite-Retrieve-Read:} A framework that explicitly separates query rewriting from retrieval, using an LLM to reformulate queries before searching \cite{rewrite_retrieve_read}.

\textbf{Query Optimization:} Systematic approaches to query transformation, including expansion with synonyms, simplification of complex queries, and normalization of linguistic variations \cite{query_optimization_survey}.
...
\textbf{RQ-RAG (Refining Queries for RAG):} Introduces an "active generator" architecture where the LLM iteratively refines queries based on retrieval results, learning when to stop searching \cite{rqrag}. This approach treats query refinement as a reinforcement learning problem.

\textbf{Self-RAG:} Incorporates special tokens that enable the model to critique retrieval quality and decide whether to retrieve additional information \cite{selfrag}.

\textbf{LevelRAG:} Implements a two-tiered architecture with high-level planning (query decomposition, tool selection) and low-level execution (sparse, dense, and web search), demonstrating the value of orchestrating multiple retrieval strategies \cite{levelrag}.
...
\textbf{Embedding Models:} Multilingual embedding models like BGE-m3 have shown reasonable performance on Arabic retrieval tasks. The MIRACL benchmark \cite{miracl} provides standardized evaluation for Arabic information retrieval.

\textbf{Chunking Strategies:} Research by Alsubhi et al. \cite{alsubhi2025} demonstrates that sentence-aware chunking outperforms fixed-size chunking for Arabic, suggesting that linguistic structure matters for retrieval quality.
