...
The software environment consisted of Python 3.10, PyTorch 2.0+, the Hugging Face Transformers library (v4.30+), Pyserini (v0.22.1) for index access, and FAISS for dense vector search. The complete implementation, including the experiment notebooks and the retrieval, fusion and evaluation modules, is publicly available; the repository is given in Appendix~\ref{app:repo}, and the key components of the CSQE pipeline are reproduced in Appendix~\ref{app:code}. For models exceeding GPU memory in native precision, NF4 quantisation was applied using the \texttt{bitsandbytes} library \cite{dettmers_2023_qlora}, following the Quantised Low-Rank Adaptation (QLoRA) quantisation scheme.
...
The initial Query2Doc experiment used Qwen~2.5~3B~Instruct (described in Appendix~\ref{app:model_details}) as the generation model. A single system prompt was used across all experiments: it instructs the model to write a passage answering the given query, forbids it from asking the user for clarification, and requires the response to be in Arabic. The prompt is reproduced verbatim in Appendix~\ref{app:code_prompts}.
...
\begin{table}[htbp]
    \centering
    \caption{Model configurations for the Query2Doc comparison experiments. All models used \texttt{max\_new\_tokens}~=~128 and \texttt{top\_p}~=~0.9, with the exception of Qwen3-4B, which used the Qwen3 non-thinking sampling recommendation of \texttt{top\_p}~=~0.8 and \texttt{top\_k}~=~20.}
...
Four of the ten models required model-specific engineering workarounds before they could be run under the standardised protocol: a batching bug in Falcon-H1-3B's hybrid architecture, a precision and input-signature constraint in Jais-2-8B, reasoning-token suppression in the Qwen3 models, and a tokenizer artefact in ALLaM-7B. Each workaround, and the reason it was necessary, is documented in Appendix~\ref{app:model_issues}; the ALLaM tokenizer artefact in particular is revisited in Section~\ref{sec:res_dropped}, where its effect on retrieval performance is quantified.
...
The routine that assembles the expanded query from the four generated samples is reproduced in Appendix~\ref{app:code_assembly}, and the full set of hyperparameters is listed in Appendix~\ref{app:code_config}. The configuration used for the primary CSQE experiment was: $k=5$ first-pass documents, 2 corpus-grounded samples, 2 blind samples, $\alpha=4$, temperature 1.0, \texttt{max\_new\_tokens}~=~128 per sample, using Aya Expanse 8B (Section~\ref{sec:aya}) in BF16 precision on an A100 GPU. Figure~\ref{fig:csqe_pipeline} illustrates the three-stage CSQE pipeline alongside a worked example.
...
For corpus-grounded generation, the model was given a system prompt distinct from the blind Query2Doc prompt of Section~\ref{sec:meth_q2d_llm}. Where the blind prompt asks the model to \emph{write} a passage from its own parametric knowledge, the corpus-grounded prompt casts it as an information-retrieval assistant whose task is to \emph{examine} the retrieved documents and \emph{extract} the key sentences relevant to the query, responding in Arabic. This extract-rather-than-generate framing is what ties the expansion to the corpus. Both prompts are reproduced verbatim in Appendix~\ref{app:code_prompts}.
...
This chapter has described the experimental methodology adopted throughout the thesis. The Arabic subset of the MIRACL benchmark was established as the evaluation corpus, and independent dense (mDPR) and sparse (BM25S) baselines were implemented and evaluated under a common protocol. A quantitative and qualitative error analysis of those baselines was then defined in order to identify the failure modes that QE must address. The Query2Doc technique was adapted for Arabic zero-shot application, together with the batching, quantisation and two-notebook arrangements required to execute it on freely available cloud GPUs, and a standardised protocol was specified for comparing the candidate generators. Finally, the chapter set out the query repetition scheme devised to counter term dilution in sparse retrieval, the hybrid fusion methods, the CSQE pipeline with its component ablation and retriever-specific application strategies, and the per-query error analysis used to interpret the behaviour of the final system.
