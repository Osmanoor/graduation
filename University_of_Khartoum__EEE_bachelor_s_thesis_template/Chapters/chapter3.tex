...
The configuration used for the primary CSQE experiment was: $k=5$ first-pass documents, 2 corpus-grounded samples, 2 blind samples, $\alpha=4$, temperature 1.0, \texttt{max\_new\_tokens}~=~128 per sample, using Aya Expanse 8B (Section~\ref{sec:aya}) in BF16 precision on an A100 GPU.
...
For each configuration, both RRF ($k=20$) and CC ($\alpha$ swept over $\{0.1, \ldots, 0.9\}$) were evaluated. This design isolates the effect of retriever-specific query representation and determines whether asymmetric expansion outperforms symmetric application.
