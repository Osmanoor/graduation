...
The generation parameters were configured as summarised in Table~\ref{tab:q2d_params}.

\begin{table}[htbp]
    \centering
    \caption[Initial Query2Doc generation parameters]{LLM generation parameters for the initial Query2Doc experiment.}
...
\begin{table}[htbp]
    \centering
    \caption[Model configurations for the Query2Doc comparison]{Model configurations for the Query2Doc comparison experiments. All models used \texttt{max\_new\_tokens}~=~128 and \texttt{top\_p}~=~0.9, with the exception of Qwen3-4B, which used the Qwen3 non-thinking sampling recommendation of \texttt{top\_p}~=~0.8 and \texttt{top\_k}~=~20.}
...
\begin{enumerate}
    ...
    \item \textbf{Corpus-grounded expansion.} All five retrieved documents are numbered and placed in a \emph{single} prompt alongside $q$, under a CSQE-specific system prompt instructing the extraction of the key sentences that make each document relevant. Two samples are drawn from this prompt at temperature 1.0.
...
The routine that assembles the expanded query from the four generated samples is reproduced in Appendix~\ref{app:code_assembly}, and the full set of hyperparameters is listed in Appendix~\ref{app:code_config}. The configuration used for the primary CSQE experiment was: $k=5$ first-pass documents, 2 corpus-grounded samples, 2 blind samples, $\alpha=4$, temperature 1.0, \texttt{max\_new\_tokens}~=~128 per sample, using Aya Expanse 8B (Section~\ref{sec:aya}) in BF16 precision on an A100 GPU. Figure~\ref{fig:csqe_pipeline} illustrates the four-stage CSQE pipeline.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{fig_3_8_csqe_aigen_v5a.png}
  \caption[CSQE pipeline]{CSQE pipeline. \textbf{Stage 1:} BM25 retrieves the top-$k_1=5$ passages $D_1 \ldots D_5$ for query $q$. \textbf{Stage 2:} Aya Expanse 8B generates two independent, parallel branches at temperature 1.0 (128 tokens each) --- a \emph{blind} branch that sees $q$ only and generates $b_1, b_2$ from parametric knowledge, and a \emph{corpus-grounded} branch that sees $D_1 \ldots D_5$ and extracts $c_1, c_2$. \textbf{Stage 3:} plain string concatenation (no scoring or re-ranking) assembles $q_{\text{CSQE}} = q\,q\,q\,q + c_1 + c_2 + b_1 + b_2$, with $q$ repeated $\alpha=4$ times to counter BM25 term dilution. \textbf{Stage 4:} $q_{\text{CSQE}}$ drives a fresh BM25 pass over the full corpus, yielding NDCG@10~$=0.616$ (BM25 with raw $q$: $0.462$).}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{fig_3_9_best_system_aigen_v2a.png}
  \caption[Best system architecture]{Architecture of the thesis's best-performing system. The full CSQE-expanded query, $q_{\text{CSQE}} = q\times4 + c_1 + c_2 + b_1 + b_2$ (Equation~\ref{eq:csqe_query}), is fed to BM25, while the original, unexpanded query is fed to mDPR; the two ranked lists are fused with RRF ($k=20$), yielding the headline NDCG@10 of 0.714.}
...
This classification, together with the aggregate comparison and the query-length and first-pass splits defined above, forms the interpretive framework applied to the final system in Chapter~\ref{chap:results}.
