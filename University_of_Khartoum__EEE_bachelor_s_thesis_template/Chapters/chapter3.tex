...
\noindent where $c_i$ denotes corpus-grounded expansion samples, $b_i$ denotes blind expansion samples, and $\alpha$ is the query repetition factor (Equation~\ref{eq:query_repetition}).
...
\begin{itemize}
    \item \textbf{Experiment 013c (Corpus-only):} \texttt{num\_corpus\_samples}~=~4, \texttt{num\_blind\_samples}~=~0. This variant isolates the contribution of first-pass grounding by using only corpus-grounded expansions.
    \item \textbf{Experiment 013d (Blind-only):} \texttt{num\_corpus\_samples}~=~0, \texttt{num\_blind\_samples}~=~4. This variant isolates the contribution of blind generation, equivalent to a 4-sample Query2Doc with repetition.
    \item \textbf{Experiment 013 (Full CSQE):} \texttt{num\_corpus\_samples}~=~2, \texttt{num\_blind\_samples}~=~2. The combined system as described above.
\end{itemize}
...
The CSQE-expanded query ($q_{\text{CSQE}}$) is substantially longer than the original query ($\sim$1,500 characters vs. $\sim$30 characters). Dense encoders such as mDPR (Section~\ref{sec:mdpr}) were trained on short natural-language queries, raising the hypothesis that long expanded queries may degrade embedding quality. To test this hypothesis, three fusion configurations were defined:

\begin{itemize}
    \item \textbf{Config A:} BM25 receives the CSQE-expanded query; Dense receives the original query. \emph{Hypothesis:} BM25 benefits from vocabulary breadth in the long expansion, while Dense performs best with the short query it was trained on.
    \item \textbf{Config B:} BM25 receives the original query; Dense receives the CSQE-expanded query. \emph{Hypothesis:} the semantic signal in the CSQE expansion improves dense encoding.
    \item \textbf{Config C:} Both BM25 and Dense receive the CSQE-expanded query. \emph{Hypothesis:} maximum expansion applied uniformly should yield the highest performance.
\end{itemize}
