...
\textbf{Retriever-specific query representation is critical.} A key finding is that applying CSQE asymmetrically---only to the BM25 retriever---outperformed applying it to both retrievers (BM25-expanded RRF: 0.7137 > Both-expanded RRF: 0.6936). BM25 benefits from CSQE's long, vocabulary-rich expansion; the mDPR encoder, trained on short queries, degrades under long expansion inputs. This retriever-specific representation principle has practical implications for similar hybrid retrieval pipelines (Section~\ref{sec:res_csqe_hybrid}).
