...
\begin{enumerate}[label=\arabic*.]
    ...
    \item \textbf{BM25 query repetition and retriever-specific strategies.} \emph{Implemented in Phase~4.} The query repetition technique was investigated and fully resolved the BM25 term-dilution problem for all nine models (Section~\ref{sec:res_repetition}). Retriever-specific expansion strategies were further developed through the asymmetric CSQE application described in Section~\ref{sec:res_csqe_hybrid}.
    ...
    \item \textbf{Hybrid retrieval with query enhancement.} \emph{Implemented in Phase~4.} Hybrid BM25--Dense fusion was combined with corpus-steered query expansion, achieving 0.7137 NDCG@10---the best result in this thesis (Section~\ref{sec:res_csqe_hybrid}).
