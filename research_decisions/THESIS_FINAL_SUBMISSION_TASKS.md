...
- [x] **D3 — Build the code appendix** — **Owner: Elhaj** `[AI]` (M) — **DONE 2026-08-02** (executed together with the D2 table/prose moves)
  ...
  **ToC treatment (Osman, 2026-08-02):** the appendices are listed **one line each, like the Bibliography** — no section breakdown. Implemented with `\addtocontents{toc}{\protect\setcounter{tocdepth}{0}}` immediately after `\appendix`, which defers the change into the `.toc` so Chapters 1–5 keep their full subsection listing. Rendered ToC tail now reads: `Bibliography 97 · A Model Details 103 · B Extended Result Tables 110 · C Implementation Code 113`. *(The `.toc` file still contains the section lines; `tocdepth` suppresses them at typeset time. Check the PDF, not the `.toc`.)*
